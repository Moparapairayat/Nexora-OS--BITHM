...
export function LandingPage() {
  ...
  useEffect(() => {
    ...
    const speakWelcome = () => {
      ...
      // 1. Play a deep, cinematic warm ambient pad sweep using Web Audio API
      try {
        ...
        if (AudioContextClass) {
          ...
          // Deep, low-frequency C Major 9 chord for a rich, organic pad sweep (C3, E3, G3, B3, D4)
          const freqs = [130.81, 164.81, 196.00, 246.94, 293.66];
          
          // Low cutoff frequency (450Hz) to keep the pad incredibly warm, soft, and deep
          const filter = audioCtx.createBiquadFilter();
          filter.type = "lowpass";
          filter.frequency.setValueAtTime(450, now);
          filter.frequency.exponentialRampToValueAtTime(120, now + 3.0);
          
          // Main volume node with smooth, slow fade-in and long ambient tail
          const mainGain = audioCtx.createGain();
          mainGain.gain.setValueAtTime(0, now);
          mainGain.gain.linearRampToValueAtTime(0.18, now + 0.4); // Deep, enveloping presence
          mainGain.gain.exponentialRampToValueAtTime(0.001, now + 3.5);
          
          filter.connect(mainGain);
          mainGain.connect(audioCtx.destination);
          
          // Play a slow, breathing arpeggiator pad
          freqs.forEach((f, index) => {
            const osc = audioCtx.createOscillator();
            const oscGain = audioCtx.createGain();
            
            osc.type = "sine"; // Pure, organic wave
            osc.frequency.setValueAtTime(f, now);
            
            const delay = index * 0.09; // Slow, breathing arpeggio delay (90ms)
            
            oscGain.gain.setValueAtTime(0, now);
            oscGain.gain.setValueAtTime(0, now + delay);
            oscGain.gain.linearRampToValueAtTime(0.3, now + delay + 0.25);
            ...
            osc.start(now + delay);
            osc.stop(now + delay + 3.8);
          });
        }
      } catch (e) {
        console.warn("Deep pad Web Audio blocked or not supported", e);
      }

      // 2. Play the deep, heart-touching male voice (delayed to match the pad peak)
      setTimeout(() => {
        if (!window.speechSynthesis) return;

        // Cancel any current/pending speech
        window.speechSynthesis.cancel();

        // Warm, comforting and deep welcoming script
        const utterance = new SpeechSynthesisUtterance(
          "Welcome to Nexora O S. We are so glad you are here. Let's make learning beautiful."
        );
        
        // Calm, deliberate, and deep reassuring voice attributes (pitch 0.85, rate 0.82)
        utterance.rate = 0.82;
        ...
        // Priority list of natural/high-quality Male voices
        const priorityKeywords = [
          "guy",       // Microsoft Guy (natural male)
          "ryan",      // Microsoft Ryan (natural male)
          "oliver",    // Microsoft Oliver (natural male)
          "alex",      // Apple Alex (premium natural male on macOS)
          "daniel",    // Apple Daniel (clear British male)
          "google us english male",
          "google uk english male",
          "google male",
          "david",     // Microsoft David (classic Windows desktop male)
          "male"       // fallback to any voice labeled male
        ];
        ...
        if (selectedVoice) {
          ...
          const fallbackMale = englishVoices.find((v) => v.name.toLowerCase().includes("male"));
          utterance.voice = fallbackMale || englishVoices[0];
        }
    ...
    // Try speaking on mount
    speakWelcome();

    // Since modern browsers restrict voice autoplay until first interaction,
    // we listen to click or touch events and trigger speech once.
    ...
    // Handle asynchronously loaded voices (e.g. Chrome)
